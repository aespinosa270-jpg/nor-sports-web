...
export const ProductCard = ({ product }: { product: Product }) => {
    ...
    const currentVariant = product.variants?.[selectedVariant] || {
        colorName: "N/A", colorHex: "#000", image: ""
    };
            ...
            >
                ...
                <div className="relative aspect-[3/4] w-full overflow-hidden bg-[#f5f5f5]">
                    ...
                    <div className={`absolute inset-0 bg-black/5 flex items-end justify-center pb-6 transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-0"}`}>
                        <button
                            onClick={(e) => { e.preventDefault(); alert("Quick Add Logic Coming Soon"); }}
                ...
                <div className="flex flex-col gap-1">
                    <div className="flex justify-between items-start">
                        ...
                        <span className="font-mono text-xs text-black">${product.price.toLocaleString()}</span>
                    </div>
                    ...
                    <div className="flex gap-2 mt-2 h-6">
                        {product.variants?.map((variant, index) => (
                            <button
                                key={index}
                                onClick={(e) => { e.preventDefault(); setSelectedVariant(index); }}
                                className={`w-4 h-4 border transition-all duration-200 ${selectedVariant === index ? "border-black scale-110 shadow-sm" : "border-transparent hover:border-gray-300"}`}
